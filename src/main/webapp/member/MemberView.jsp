...
<div class="wrap">
	...
	<h1 style="text-align:center; padding-top:100px;">회원 리스트</h1>
		...
		<c:forEach var="s" items="${list}">
			<tr>
				<th><a href="${contextPath}/member/MemberInfo.jsp?id=${s.id}">${s.id}</a></th><th>${s.pwd}</th><th><a href="${contextPath}/member/MemberInfo.jsp?id=${s.id}">${s.name}</a></th><th>${s.addr}</th>
			</tr>
		</c:forEach>
